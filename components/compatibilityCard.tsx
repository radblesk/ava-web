...
export default function CompatibilityCard({
  ...
  return (
    ...
    >
      <p
        className={`relative text-center text-[8px] leading-3 font-semibold uppercase sm:text-[8px] ${atLaunch ? "text-sky-500" : "text-orange-500"}`}
      >
        {atLaunch ? t("atLaunch") : t("later")}
        <span
          style={{ animationDelay: `${delay}s` }}
          className={`animate-flicker absolute inset-0 text-center text-[8px] leading-3 font-semibold uppercase blur-sm sm:text-[8px] ${atLaunch ? "text-sky-600 dark:text-sky-400" : "text-orange-600 dark:text-orange-400"}`}
        >
          {atLaunch ? t("atLaunch") : t("later")}
        </span>
      </p>
